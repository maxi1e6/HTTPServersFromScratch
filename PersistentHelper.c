...
void parseRequest(char buff[], int clientSocket)
{
	...
	char *filename = parseRequests1stLine(first_line, clientSocket); // Stephan's function (edited)
...
{
	printf("parsing client's request headers:\n"); // TODO: remove
	char *delim_req = "\r\n";
	char *end_str;
	// skip the 1st line:
	char *token = strtok_r(buff, delim_req, &end_str);
...
char *parseRequests1stLine(char *first_line, int clientSocket)
{
	...
	while (token != NULL)
	{
		...
		case 1:
			...
			break;
		case 2:
			// CHANGE TO 1.0 WHEN DONE TESTING
...
// finds the path of the file we need to respons with
...
// create a response based a clients' request's parameters, where:
// 1.
